...
! Modified to Parallel by Charles Swartz Thu Aug 2, 2012
!
! This file is distributed under the terms of the
! GNU General Public License. See the file `License'
! in the root directory of the present distribution,
...
PROGRAM fpmd_postproc
  ...
  USE mp,            ONLY : mp_start, mp_bcast, mp_barrier
  ...
  !Charles Swartz
  INTEGER            :: me, nks, total_ks, per_proc
  CHARACTER(len=256) :: ks_outdir, x1, print_state_root 
  CHARACTER(len=256) :: outdir !Outdire from the previsious calculatiosn
  ...
  CALL io_global_start( mpime, root )
   ...
   IF(ionode) Then
      write(stdout,'("------------------------------------------------")') 
      write(stdout,'("|          Modified Parallel cppp.x            |")')
      write(stdout,'("|                                              |")')
      write(stdout,'("|     NOTE: For use mainly with multiple       |")')
      write(stdout,'("|    KS-States. NOT intended for ldynamics     |")')
      write(stdout,'("|                                              |")')
      write(stdout,'("------------------------------------------------")') 
      Write(stdout,*) ''
      write(stdout,*) 'total procs: ', nproc
   ENDIF

   !write(6,*)  'hello I am ', mpime
  ...
  IF(ionode) THEN
   ...
   print_state_root = print_state
  ENDIF

  !Broadcast all variables
  Call mp_bcast( prefix, ionode_id)
  Call mp_bcast( fileout, ionode_id)
  Call mp_bcast( output, ionode_id)
  Call mp_bcast( outdir, ionode_id)
  Call mp_bcast( lcharge, ionode_id)
  Call mp_bcast( lforces, ionode_id)
  Call mp_bcast( lpdb, ionode_id)
  Call mp_bcast( lrotation, ionode_id)
  Call mp_bcast( ns1, ionode_id)
  Call mp_bcast( ns2, ionode_id)
  Call mp_bcast( ns3, ionode_id)
  Call mp_bcast( np1, ionode_id)
  Call mp_bcast( np2, ionode_id)
  Call mp_bcast( np3, ionode_id)
  Call mp_bcast( print_state_root, ionode_id)
  Call mp_bcast( atomic_number, ionode_id)
  Call mp_bcast( nframes, ionode_id)
  Call mp_bcast( ndr, ionode_id)
  Call mp_bcast( charge_density, ionode_id)
  Call mp_bcast( lbinary, ionode_id)
  Call mp_bcast( ks_outdir, ionode_id)
  Call mp_bcast( total_ks, ionode_id)

  
  outdir = outdir // '/'
  ks_outdir = ks_outdir // '/'

   ! set file names, from previous run
   !
   IF(ionode) THEN
      tmp_dir = TRIM(outdir)
   ...
   IF(ldynamics) then
      write(stdout,*) 'Error: Parallel version cannot be used with ldynamics True!'
      Stop
   ENDIF
   ...
   IF (ldynamics .AND. ( print_state_root /= ' ' ) ) THEN
   ...
   IF(ionode) CALL iotk_scan_begin( dunit, "IONS", FOUND = found )
   ...
   DO i = 1, nsp
         !
         if (ionode) CALL iotk_scan_begin( dunit, "SPECIE" // TRIM( iotk_index( i ) ), FOUND = found )
         !
         Call mp_bcast(found, ionode_id)
         !
         IF( .NOT. found ) THEN
         ...
         if(ionode) CALL iotk_scan_dat( dunit, "ATOM_TYPE", atm(i) )
         !
         ...
         !
         IF(ionode) CALL iotk_scan_end( dunit, "SPECIE" // TRIM( iotk_index( i ) ) )
         !
   END DO
   ...
   Call mp_bcast(force0, ionode_id)
   Call mp_bcast(ht0, ionode_id)
   !
   ispin = 1
   !
   !
  IF(ionode) CALL iotk_close_read( dunit )
   !
  ...
  ! charge density
  IF ( lcharge .OR. print_state_root /= ' ' ) THEN
     IF (ns1 == 0) ns1 = nr1
     IF (ns2 == 0) ns2 = nr2
     IF (ns3 == 0) ns3 = nr3
  ...
  END IF
   ...
   !Open Main, singular, output files on ionode. These are not the
   !Multiple KS files. 
   IF (ionode) THEN
   ...
   ! Unit for KS states
   ! Each Processor will open a different KS Output file
   ksunit = 16

   ! XSF file header
   !
   IF ( output == 'xsf' .AND. ionode ) THEN
   ...
   DO n = 1, nframes
      ...
      !
      IF (ionode) CALL read_fpmd( lforces, lcharge, lbinary, cunit, punit, funit, dunit, &
                     natoms, nr1, nr2, nr3, ispin, at, tau_in, force, &
                     rho_in, prefix, tmp_dir, ndr, charge_density )

      Call mp_bcast(at, ionode_id)
      Call mp_bcast(tau_in, ionode_id)
      if (lforces) Call mp_bcast(force, ionode_id)
      Call mp_bcast(rho_in, ionode_id)
      ...
      IF (ionode ) then
         WRITE(stdout,'(2x,"Cell parameters (Angstroms):")')
         WRITE(stdout,'(3(2x,f10.6))') ((at(i, j), i=1,3), j=1,3)
         !
         WRITE(stdout,'(2x,"Atomic coordinates (Angstroms):")')
         WRITE(stdout,'(3(2x,f10.6))') ((tau_in(i, j), i=1,3), j=1,natoms)
         WRITE(stdout, *) 
      ENDIF
   ...
   !Number of jobs per processor
   per_proc = Ceiling(total_ks/real(nproc))   

   Call mp_barrier()
   !This is the multiple loop part
   IF ( print_state_root /= ' ' ) THEN 
      do nks = 1,per_proc 
   
         me = mpime + (nks-1) * nproc + 1
         if (me > total_ks) exit

         !Update the file numbers
         write(x1,'(I0)') me
         print_state = TRIM(outdir)//TRIM(print_state_root)//TRIM(x1)

         !
         CALL read_density( TRIM( print_state ) // '.dat', dunit, nr1, nr2, nr3, rho_in, lbinary )
         CALL scale_charge( rho_in, rho_out, nr1, nr2, nr3, ns1, ns2, ns3, np1, np2, np3 )
         !
         IF (output == 'xsf') THEN
           print_state = TRIM(print_state_root)//TRIM(x1)
           ! write data as XSF format
           OPEN( unit = ksunit, file = TRIM(ks_outdir)//'/' // TRIM( print_state ) // '.xsf' )
   ...
   if (ionode) CALL write_pdb( bunit, at, tau_out, natoms, ityp, euler, lrotation )

   Call mp_barrier()
   if(ionode) write(6,'(/,3x, "...Program Complete")')
...
SUBROUTINE read_density( filename, dunit, nr1, nr2, nr3, rho, lbinary )
   ...
   REAL(DP), ALLOCATABLE :: rho_plane(:)
     ...
     !
     WRITE(6,'(2x, "Reading density from: ", A80)' ) filename 
     !
     ...
     ! print some info
     WRITE(6,'(2x, "---------------------", /, 3x, "KS state =", A30, /, &      
                  3x,"Density grid:",/, 3(3x,i6), /, 3x,&
                  "Minimum and maximum values:", /, 3(3x,1pe12.4))') &  
                  filename, nr1, nr2, nr3, rhomin, rhomax 
...
SUBROUTINE write_xsf( ldynamics, lforces, lcharge, ounit, n, at, &
  ...
  INTEGER :: i, j, ix, iy, iz, ind_x(6), ind_y(6), ind_z(6), count
  ...
  IF (lcharge) THEN
     ...
     !-------------------------------------------------------------------------
     !Charles Swartz
     ! The above charge density out putproduces the same pattern as 
     ! below (From pp.x), however, The code below can have a 'wrap-around effect'
!     count=0
      !DO i3=0,nr3
!      DO iz=1,nr3
         !iz = mod(i3,nr3) + 1
