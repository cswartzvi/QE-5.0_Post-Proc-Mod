...
PROGRAM pp
  ...
  !
  CALL extract (filplot, plot_num, multi_kband)
  !
  CALL chdens (filplot, plot_num, multi_kband)
  !
  CALL stop_pp()
  !
END PROGRAM pp
!
!-----------------------------------------------------------------------
SUBROUTINE extract (filplot,plot_num, multi_kband)
  !-----------------------------------------------------------------------
  !
  ...
  !Charles Swartz
  LOGICAL, INTENT(inout) :: multi_kband
  INTEGER :: start_kband, end_kband, total_kbands

  NAMELIST / inputpp / outdir, prefix, plot_num, sample_bias, &
       spin_component, z, dz, emin, emax, kpoint, kband, &
       filplot, lsign, epsilon, multi_kband, start_kband, &
       end_kband !Charles Swartz
  !
  ...
  CALL mp_bcast( multi_kband, ionode_id )
  CALL mp_bcast( start_kband, ionode_id )
  CALL mp_bcast( end_kband, ionode_id )
  !
  !
  !Charles Swartz
  IF (plot_num /= 7 .and. multi_kband == .true.) CALL errore ('postproc', &
          'multi_kband only allowed with plot_num = 7', ABS (plot_num) )
  if(multi_kband ) then
    kband = start_kband
  else
    end_kband=kband
...
!  CALL init_us_1 ( )
  ...
  CALL punch_plot (filplot, plot_num, sample_bias, z, dz, &
       ...
       multi_kband, start_kband, end_kband ) !Charles Swartz
  !
