...
!
!-----------------------------------------------------------------------
SUBROUTINE chdens (filplot,plot_num, multi_kband)
  !-----------------------------------------------------------------------
  ...
  CHARACTER (len=256) :: fileout, fileout_root
  ...
  CALL mp_bcast( fileout_root, ionode_id )
  ...
  WRITE( stdout, '(/5x,"Starting Second Loop )')
  WRITE( stdout, '(5x, "-----------------------------------")')
  ...
  DO
    ...
    if(multi_kband) then
      ...
      if (ios /= 0 ) then
        ...
        WRITE(x1,"(I0)") ncount
        fileout = TRIM(fileout_root)//TRIM(x1)//'.xsf'
        CALL mp_bcast( filepp, ionode_id )
        CALL mp_bcast( fileout, ionode_id )
      endif
    endif   
...
!       nproc_pool=1
  ...
  !
  !Charles Swartz
  if (ionode) close(ounit)
  if (.not. multi_kband) exit
  ncount = ncount + 1 
ENDDO
  !

  Write(stdout,'(/7x, "...Plotting Complete")') 
  !
