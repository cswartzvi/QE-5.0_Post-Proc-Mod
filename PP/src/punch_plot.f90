...
SUBROUTINE punch_plot (filplot, plot_num, sample_bias, z, dz, &
  ...
  INTEGER, INTENT(IN) :: start_kband, end_kband
  INTEGER :: ncount=0
  CHARACTER(len=100) :: x1
  CHARACTER(len=100) :: filplot_root
...
!Main loop Charles Swartz
do

  if(multi_kband) then
    !update counters
    ncount = ncount + 1
    kband = start_kband + (ncount-1)
    !New filplot
    WRITE(x1,"(I0)") ncount
    filplot = TRIM(filplot_root)//TRIM(x1)
    if(ionode) write(30,*) TRIM(filplot)
  endif

  IF (ncount == 1) then
     WRITE( stdout, '(/5x,"Calling punch_plot, plot_num = ",i3)') plot_num
     WRITE( stdout, '(/5x,"Starting First Loop" )')
     WRITE( stdout, '(5x, "-----------------------------------")')
  ENDIF
  IF (plot_num == 7 ) &
  ...
  !Charles Swartz
    if( ( multi_kband .and. (kband == end_kband)) .or. (.not. multi_kband) ) exit

enddo !Charles Swartz
